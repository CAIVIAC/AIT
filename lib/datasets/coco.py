...
from lib.utilities import Bar, FillingSquaresBar
...
class coco(imdb):
    ...
    def gt_roidb(self):
        ...
        cache_file = osp.join(self.cache_path, '{name}_gt_roidb.pkl'.format(name=self.name))

        if osp.exists(cache_file):
            with open(cache_file, 'rb') as fid:
                [roidb, self.cat_data] = pickle.load(fid)
            printer(bold_info='Loaded Cache File: ', bold_color='blue',
                    prnt_info='{} gt roidb loaded from {}'.format(self.name, cache_file))
            return roidb

        gt_roidb = [self._load_coco_annotation(index)
                    for index in self._image_index]

        # print([gt_roidb, self.cat_data])
        # print('gt')
        # exit()
        with open(cache_file, 'wb') as fid:
            pickle.dump([gt_roidb,self.cat_data], fid, pickle.HIGHEST_PROTOCOL)
        # print('wrote gt roidb to {}'.format(cache_file))
        printer(bold_info='Write ', bold_color='yellow',
                prnt_info='gt roidb to {}'.format(cache_file))
        return gt_roidb
    ...
    def _do_detection_eval(self, res_file, output_dir, save_results=True, det_results=None):
        ann_type = 'bbox'
        ...
        if save_results:
            ...
            coco_dt = self._COCO.onlineRes(det_results)

        cocoEval = customCOCOeval(self._COCO, coco_dt, iouType="bbox")
        ...
        printer(bold_info='Write COCO eval results to: ', bold_color='blue',
                prnt_info='{}'.format(eval_file))
    ...
    def _coco_results_one_category(self, boxes, cat_id):
        ...
        for im_ind, index in enumerate(self.image_index):
            ...
            if len(dets) == 0:
                continue
            dets = np.array(dets).astype(float)
            scores = dets[:, -1]
            xs = dets[:, 0]
            ys = dets[:, 1]
            ws = dets[:, 2] - xs + 1
    ...
    def _write_coco_results_file(self, all_boxes, res_file, save_results=True):
        # [{"image_id": 42,
        #   "category_id": 18,
        ...
        print('')
        bar = FillingSquaresBar('[ {title} ]'.format(title=color('Collecting Results', 'blue')),\
                max=self.num_classes - 1) # exclude background
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            ...
            bar.suffix = \
                ...
                ' | Class: {cls}'\
                .format(
                    cnt=cls_ind, size=self.num_classes - 1,
        ...
        if save_results:
            ...
            printer('Writing results json to:', prnt_info=' {}'.format(res_file))
    ...
    def evaluate_detections(self, all_boxes, output_dir, save_results=True):
        ...
        res_file += '.json'
        det_results = self._write_coco_results_file(all_boxes, res_file, save_results=save_results)
        # Only do evaluation on non-test sets
        if self._image_set.find('test') == -1:
            self._do_detection_eval(res_file, output_dir, save_results=save_results,\
                det_results=None if save_results else det_results)
...
class customCOCOeval(COCOeval):
    def summarize(self, class_index=None, verbose=True):
        '''
        ...
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100 ):
            ...
            if verbose:
                ...
                # exit()
                info = [
                    '{s_title:<18}'.format(s_title=titleStr),
                    '{s_type}'.format(s_type=typeStr),
                    '{s_iou:<9}'.format(s_iou=iouStr),
                    '{s_area:>6s}'.format(s_area=areaRng),
                    '{s_maxdets:>3d}'.format(s_maxdets=maxDets),
                    '{s_mean:.3f}'.format(s_mean=mean_s),
                ]
                return mean_s, info
            else:
                return mean_s, []
        def _summarizeDets(verbose=True):
            stats = np.zeros((12,))
            stats_info = [list() for _ in range(12)]
            stats[0], stats_info[0] = _summarize(1)
            stats[1], stats_info[1] = _summarize(1, iouThr=.5, maxDets=self.params.maxDets[2])
            stats[2], stats_info[2] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])
            stats[3], stats_info[3] = _summarize(1, areaRng='small', maxDets=self.params.maxDets[2])
            stats[4], stats_info[4] = _summarize(1, areaRng='medium', maxDets=self.params.maxDets[2])
            stats[5], stats_info[5] = _summarize(1, areaRng='large', maxDets=self.params.maxDets[2])
            stats[6], stats_info[6] = _summarize(0, maxDets=self.params.maxDets[0])
            stats[7], stats_info[7] = _summarize(0, maxDets=self.params.maxDets[1])
            stats[8], stats_info[8] = _summarize(0, maxDets=self.params.maxDets[2])
            stats[9], stats_info[9] = _summarize(0, areaRng='small', maxDets=self.params.maxDets[2])
            stats[10], stats_info[10] = _summarize(0, areaRng='medium', maxDets=self.params.maxDets[2])
            stats[11], stats_info[11] = _summarize(0, areaRng='large', maxDets=self.params.maxDets[2])

            if verbose:
                title = [['Metric', 'AP/AR', 'IoU Range', 'area', 'maxDets', 'Score']]

                table = DoubleTable(title + stats_info, ' Performance on COCO datasets ')
                table.justify_columns = {
                    0: 'left', 1: 'center', 2: 'left', 3: 'right', 4: 'right', 5: 'center'
                }
                print('')
                print(table.table)
                print('')
