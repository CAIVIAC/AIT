...
def parse_directory(frame_folders, key_func=lambda x: x[-11:],
    ...
    print('parse {} videos'.format(len(frame_folders)))
    ...
    for i, f in enumerate(frame_folders):
        ...
        k = key_func(f) # video_name: 
        """
        - f: /home/heyen/mountFolder/database/Frames/ActivityNet-1.3/v_M1hPI8WvySo
        - k: M1hPI8WvySo
        """
...
def dump_window_list(video_info, named_proposals, frame_path, name_pattern, allow_empty=False, score=None):
    ...
    """ video_info.duration: duration is defined in activitynet json file """
    ...
    gt_windows = [(x[0]+1, int(x[1][0] * real_fps), int(x[1][1] * real_fps)) for x in gt_w] # add 1 to label
    ...
    for pr in named_proposals:
        real_start = int(pr[3] * real_fps) # turn to frame
        real_end = int(pr[4] * real_fps) # turn to frame
        label = pr[0]
        overlap = pr[1]
        overlap_self = pr[2]
    ...
    - NUM_UNITS: the next line has a number indicating the total units of time for this video.
        The unit can be a frame, a second, or a 1 for the normalized proposal files.
    ...
    - (CLASS START END) x NUM_GT: Then go NUM_GT lines of groundtruth action instances.
        Each instance has a CLASS id, the START and END in the unit used by this proposal file.
        For example, in SSN we use the frame unit.
        So here START and END will denote the starting and ending frame of the instance.
        In the provided normalized proposal files, the unit is 1.
        So the START and END will be a decimal number from 0 to 1.
        Actually, the [gen_proporal_list.py][https://github.com/yjxiong/action-detection/blob/master/gen_proposal_list.py]
        script is translating between these two units based on the actual number of frames extracted for each video on your machine.
    - NUM_PROP: After the groundtruth instances come the proposals, lead by the total number of proposals for this video.
    - (CLASS MAX_IOU MAX_OVERLAP START END) x NUM_PROP:
        similarly, proposals a recorded one per line for NUM_PROP lines.
        Compared with a groundtruth instance, a proposal has two more fields.
        MAX_IOU stands for the maximal intersection over union (IoU) of this proposal w.r.t. all groundtruth instances.
        MAX_OVERLAP stands for the maximal overlap with groundtruth proportional to the length of this proposal.
        Here the CLASS is for a proposal is the one from the groundtruth instance with the max IoU.

    see: https://github.com/yjxiong/action-detection/wiki/A-Description-of-the-Proposal-Files
    """
