...
import numpy as np
# from system_mask_concat.Layers import EncoderLayer, DecoderLayer

from model.system_mask_concat.Layers import EncoderLayer, DecoderLayer
...
def get_subsequent_mask(seq):
    ...
    subsequent_mask = torch.ones((1, len_s, len_s), device=seq.device).type(torch.uint8)
...
class Transformer(nn.Module):
    ...
    def forward(self, x_props, x_query):
        """
        :param src_seq
            - size: [bs=256, N_s=32] (Transformer)
            - size: [bs*num_props, C=1024, H=7, W=7] (Proposal)
        :param trg_seq
            - size: [bs=256, N_t=29] (Transformer)
            - size: [bs, C=1024, H=8, W=8] (Query)
        :var trg_vocab_size = 9521
        :var src_pad_idx = trg_pad_idx = 1
        """
