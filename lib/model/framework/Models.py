...
import numpy as np
# from framework.Layers import EncoderLayer, DecoderLayer

from model.framework.Layers import EncoderLayer, DecoderLayer
...
class Transformer(nn.Module):
    ...
    def forward(self, x_props, x_query, decode2query=True):

        d_word_vec = self.channels
        ...
        x_query = x_query.unsqueeze(1).repeat(1, num_props, 1, 1, 1) # [bs, num_props, C=512, H=7, W=7]

        x_props = x_props.view(bp, d_word_vec, -1) # Proposal: [bs * num_props, C=512, T=HW=49]
        x_query = x_query.view(bp, d_word_vec, -1) # Query:    [bs * num_props, C=512, T=HW=64]

        len_p, len_q = x_props.size(-1), x_query.size(-1)

        x_props, x_query = x_props.transpose(1, 2), x_query.transpose(1, 2) # [bp, T=HW, C=512]

        pad_mask = torch.zeros((bp, 1, len_q - len_p), dtype=torch.uint8).to(device) # False: padded words
        zpadings = torch.zeros((bp, len_q - len_p, d_word_vec)).to(device)
        if decode2query:
            src_seq, trg_seq = x_props, x_query

            src_mask = torch.ones((bp, 1, len_p), dtype=torch.uint8).to(device) # False: padded words
            src_mask = torch.cat((src_mask, pad_mask), dim=2) # [bs, 1, N_t]

            trg_mask = torch.ones((bp, 1, len_q), dtype=torch.uint8).to(device) # False: padded words
            ...
            dec_output = dec_output.view(bp, d_word_vec, h_query, w_query) # [bp, C=512, H=8, W=8]
            dec_output = self.dec_trans(dec_output) # [bp, C=1024, H=8, W=8]
        else:
            src_seq, trg_seq = x_query, x_props

            trg_mask = torch.ones((bp, 1, len_p), dtype=torch.uint8).to(device) # False: padded words
            pad_mask = torch.zeros((bp, 1, len_q - len_p), dtype=torch.uint8).to(device) # False: padded words
            trg_mask = torch.cat((trg_mask, pad_mask), dim=2) # [bs, 1, N_t]
            trg_mask = trg_mask & get_subsequent_mask(trg_mask) # [bs, N_t, N_t]

            src_mask = torch.ones((bp, 1, len_q), dtype=torch.uint8).to(device) # False: padded words

            # pad zeros
            trg_seq = torch.cat((trg_seq, zpadings), dim=1) # padding zeros: [bp, N_t, C=512]

            enc_output, *_ = self.encoder(src_seq, src_mask) # [bp, N_t, C=512]
            dec_output, *_ = self.decoder(trg_seq, trg_mask, enc_output, src_mask) # [bp, N_t, C=512]

            dec_output = dec_output[:, :h_props*w_props, :] # exclude zero paddings
            # ====================== Dimension Transform ======================  #
            dec_output = dec_output.permute(0, 2, 1).contiguous() # [bp, C=512, N_t]
            dec_output = dec_output.view(bp, d_word_vec, h_props, w_props) # [bp, C=512, H=7, W=7]
            dec_output = self.dec_trans(dec_output) # [bp, C=1024, H=7, W=7]
        return dec_output
