...
class roibatchLoader(data.Dataset):
    """flip_test: Only used in testing mode, a flag that indicates whether using flip test or not."""
    def __init__(self, roidb, ratio_list, ratio_index, query, batch_size,
                 num_classes, training=True, normalize=None, seen=True):
        self._roidb = roidb
        self._query = query
        ...
        if self._num_classes == 81: # COCO classes
            self.dataset = 'coco'
            self._cat_ids = [
        ...
        elif self._num_classes == 21: # PASCAL VOC classes
            self.dataset = 'voc'
            self._cat_ids = list(range(1, self._num_classes))
