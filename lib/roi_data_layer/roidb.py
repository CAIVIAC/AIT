...
def filter_roidb(roidb):
    ...
    printer('[Before] filtering: ',
            prnt_info='There are {} images'.format(color(str(len(roidb)), txt_color='blue')))
    i = 0
    while i < len(roidb):
    ...
    printer('[After ] filtering: ', bold_color='green',
            prnt_info='There are {} images'.format(color(str(len(roidb)), txt_color='green')))
    return roidb
...
def combined_roidb(imdb_names, training=True, seen=1):
  ...
  def get_training_roidb(imdb, training):
    ...
    if cfg.TRAIN.USE_FLIPPED and training:
      printer('Appending horizontally-flipped training examples ...')
      imdb.append_flipped_images()
      printer('Done !')


    printer('Preparing training data...')
    prepare_roidb(imdb)
  ...
  def get_roidb(imdb_name, training):
    ...
    printer('Loaded dataset `{:s}` for training'.format(imdb.name))
    imdb.set_proposal_method(cfg.TRAIN.PROPOSAL_METHOD)
    printer('Set proposal method: {:s}'.format(cfg.TRAIN.PROPOSAL_METHOD))
